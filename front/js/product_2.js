...
const produitDisplay = async () => {
    ...
    imgProduit.setAttribute('src', `${produitsData.imageUrl}`)
    imgProduit.setAttribute('alt', `${produitsData.altTxt}`)
...
const ajoutCanape = () => {
    ...
    bouton.addEventListener("click", () => {
        ...
        if (select.value && nombreArticles.value && (nombreArticles.value > 0 && nombreArticles.value <= 100)) {
            const { price, ...product } = produitsData
            const fusionProduitCouleurs = Object.assign({}, product, {
                colors: `${select.value}`,
